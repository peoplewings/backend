...
class UserProfileResource(ModelResource):    
	...
	def prepend_urls(self):
		return [
			url(r"^(?P<resource_name>%s)/(?P<profile_id>\w[\w/-]*)/wings%s$" % (self._meta.resource_name, trailing_slash()), 
				self.wrap_view('wings_collection'), name="api_list_wings"),
			url(r"^(?P<resource_name>%s)/(?P<profile_id>\w[\w/-]*)/wings/(?P<wing_id>\w[\w/-]*)%s$" % (self._meta.resource_name, trailing_slash()), 
				self.wrap_view('wings_detail'), name="api_detail_wings")		
		]

	def wings_collection(self, request, **kwargs):
		accomodation_resource = WingResource()
		return accomodation_resource.dispatch_list(request, **kwargs)  

	def wings_detail(self, request, **kwargs):
		accomodation_resource = WingResource()
