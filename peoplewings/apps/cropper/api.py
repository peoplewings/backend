...
class CropcompletedResource(ModelResource):
	...
	def post_list(self, request, **kwargs):
		...
		values = {"json": [{"src": url, "content_type_json": True, "functions": [{"params": {"width": 174,"height": 174},"name": "resize","save": {"image_identifier": image_id_big, "quality":100, "s3_destination" : {"bucket" : { "name" : settings.AWS_STORAGE_BUCKET_NAME, "location": settings.AWS_BUCKET_LOCATION}, "key" : s3_key_big}}}],"application_id": settings.BLITLINE_ID,"postback_url": postback}]}
		headers = {"Accept": "application/json", "Content-Type": "application/json"}
		req = urllib2.Request(url_blitline, json.dumps(values), headers)
 		res= urllib2.urlopen(req).read()
		...
		values = {"json": [{"src": url, "content_type_json": True, "functions": [{"params": {"width": 66,"height": 66},"name": "resize","save": {"image_identifier": image_id_med, "quality":100, "s3_destination" : {"bucket" : { "name" : settings.AWS_STORAGE_BUCKET_NAME, "location": settings.AWS_BUCKET_LOCATION}, "key" : s3_key_med}}}],"application_id": settings.BLITLINE_ID,"postback_url": postback}]}
		headers = {"Accept": "application/json", "Content-Type": "application/json"}
		req = urllib2.Request(url_blitline, json.dumps(values), headers)
 		res= urllib2.urlopen(req).read()
...
class CropbigResource(ModelResource):
	...
	def post_list(self, request, **kwargs):
		...
		try:
			...
			url = str(POST["results"][0]['s3_url'])
			url = str.replace(url, 'http://', '//', 1)
			img_id = str(POST["results"][0]['image_identifier'])
