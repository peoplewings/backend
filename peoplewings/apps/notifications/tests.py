...
class PaginationTest(TestCase):
	...
	def test_models(self):
		self.assertEqual(Notifications.objects.filter(receiver=self.pj).count(), Notifications.objects.filter(sender=self.ph).count())
		self.assertEqual(Notifications.objects.filter(sender=self.ph).count(), 5)

		self.assertEqual(Notifications.objects.filter(sender=self.pj).count(), Notifications.objects.filter(receiver=self.ph).count())
		self.assertEqual(Notifications.objects.filter(receiver=self.ph).count(), 3)
	...
	def test_pagination(self):
		...
		self.assertEqual(json.loads(r2.content)['data']['count'], 3)
		...
		#check that not providing the page parameter has the same effect as calling with page=1
		r4 = c.get('/api/v1/notificationslist?page=2', HTTP_X_AUTH_TOKEN=token, content_type='application/json')
		...
		self.assertEqual(json.loads(r4.content)['data']['count'], 3)
		self.assertEqual(json.loads(r4.content)['data']['endResult'], 3)
		self.assertEqual(json.loads(r4.content)['data']['startResult'], 3)
		self.assertEqual(len(json.loads(r4.content)['data']['items']), 1)

		# checking unexisting pages...
		r3 = c.get('/api/v1/notificationslist?page=3', HTTP_X_AUTH_TOKEN=token, content_type='application/json')
